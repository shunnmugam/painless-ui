...
class ToastContainer extends React.PureComponent {
    ...
    render() {
        ...
        return (React.createElement("div", Object.assign({ className: 'ui-toast-container ' + position + ' ' + className }, customProps), this.state.toast.map((toast) => {
            const { onClose, ...props } = toast.config;
            return React.createElement(Toast, Object.assign({ key: props.toastId, onClose: (id) => {
                    this.remove(id);
                    if (onClose) {
                        onClose(id);
                    }
