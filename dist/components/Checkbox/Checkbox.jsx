...
const Checkbox = (props) => {
    ...
    return (<label className={'ui-checkbox-container' + (rounded ? ' rounded' : '')} style={{ ...{
        ...
        <input id={id} onClick={($e) => onChangeHandler($e, props)} className={'ui-checkbox' + className + (theme && theme.className ? " " + theme.className : "")} type="checkbox" {...customProps}/>
        <span className={'checkmark ' + (rounded ? ' rounded' : '')}></span>
    </label>);
};
